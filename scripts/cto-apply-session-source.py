...
sub_once("server/session-report.ts", r'(export interface SessionReport \{.*?returning: boolean;\n\})', '''export interface SessionReport {
...
sub_once("server/session-report.ts", r'const FIRST_DAY_BACK =\n  /.*?training\\b/i;\n', '''const FIRST_DAY_BACK =
...
sub_once("server/session-report.ts", r'  const didTrain = TRAINED_PAST\.test\(s\).*?return \{ trainedToday: true, feel: readFeel\(s\), returning: RETURNING\.test\(s\) \};', '''  const weekMatch = s.match(WEEK_SESSION);
...
sub_once("server/session-report.ts", r'export function sessionReportReply\(r: SessionReport, firstName = "", totalSessions\?: number\): string \{.*?const logged = `✅ \$\{fn\}logged today\'s session\.\$\{tally\}`;', '''export function sessionReportReply(r: SessionReport, firstName = "", totalSessions?: number, weekSession?: number): string {
  ...
  const logged = `✅ ${fn}logged today's session.${tally}${split}`;''', "session reply signature", re.DOTALL)

sub_once("server/handlers/workout.ts", r'import \{ applyRetroSessionState \} from "\.\./day-ledger";', 'import { applyRetroSessionState, sessionsThisCalendarWeek } from "../day-ledger";', "day ledger import")
sub_once("server/handlers/workout.ts", r'const dupe = sessionReportReply\(report, firstName, user\.totalWorkoutsCompleted \|\| 0\)', 'const weekSession = report.weekSessionNumber || await sessionsThisCalendarWeek(user.id).catch(() => undefined);\n    const dupe = sessionReportReply(report, firstName, undefined, weekSession)', "duplicate session reply")
sub_once("server/handlers/workout.ts", r'const reply = sessionReportReply\(report, firstName, newTotal\);', 'const weekSession = report.weekSessionNumber || await sessionsThisCalendarWeek(user.id).catch(() => undefined);\n  const reply = sessionReportReply(report, firstName, undefined, weekSession);', "prose session reply")

sub_once("script/tracking-contract-tests.ts", r'  /\*\*\n   \* STAGE 3 OF THE CONTRACT — ONE WRITE OWNER, AND EVERY CONVERSATIONAL DOOR GOES THROUGH IT\.', '''  /**
  ...
  /**
   * STAGE 3 OF THE CONTRACT — ONE WRITE OWNER, AND EVERY CONVERSATIONAL DOOR GOES THROUGH IT.''', "tracking contract insertion")
