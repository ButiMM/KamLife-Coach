...
| 10 | **Admin auth = single shared secret** | ✅ HARDENED | Timing-safe compare, header-only auth, brute-force lockout now DB-backed via `adminEvents` — cross-replica and restart-safe. Per-user accounts + MFA still a v2 milestone. | `adminEvents` table tracks failed login attempts by IP; 5 failures in 15 min = 429 across all replicas. |
| 11 | **In-memory Twilio SID dedup** — `processedSids` Map is process-local | ✅ FIXED | `processedWebhooks` table (TEXT PRIMARY KEY on MessageSid) provides DB-level atomic test-and-set. `INSERT ON CONFLICT DO NOTHING` is the lock. In-memory Map retained as fast-path same-instance check. Stale rows purged nightly. | Survives Railway container restarts and multi-replica deployments. |
| 12 | **In-memory login rate-limit** — `loginAttempts` Map resets on restart | ✅ FIXED | Login handler now queries `adminEvents` table for failed attempts in the last 15 min by IP. Cross-replica: any instance that sees the attempts can enforce the lockout. In-memory Map retained as fast path. | `getFailedAttemptsDb()` + `recordFailedAttemptDb()` in `server/routes/auth.ts`. |
