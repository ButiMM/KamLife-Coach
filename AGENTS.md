...
6. **You also run the database suite. It no longer runs on GitHub.** For every open PR labelled `ready` without a `DBSUITE` comment at its current head SHA: check out that exact SHA and run the suite exactly as the `pg-acceptance` job in `.github/workflows/test.yml` defines it. That means Postgres 16 with user/password `kam`, database `kamlife`, `DATABASE_URL=postgres://kam:kam@127.0.0.1:5432/kamlife`, then `npm ci`, `npm run check`, `npm test`, `npm run db:migrate`, `npx tsx script/pg-acceptance-runner-controls.ts`, `npx tsx script/pg-acceptance-runner.ts`. Post one comment starting `DBSUITE @ <first 7 of SHA>: PASS` (with counts) or `DBSUITE @ <sha>: FAIL` (with each failing acceptance id and its first error). If you cannot run it, post `DBSUITE @ <sha>: UNAVAILABLE` and the reason. Never skip silently. Run the suite before your attack when both are due; a failing suite is the first finding.
