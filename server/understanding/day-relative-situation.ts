 ...
 *
 * This extends the existing temporal owner without writing, choosing an action, or deciding
 * what the client should do. Each beat gets its own day attribution.
 */
...
function attributionSastParts(date: Date): { year: number; month: number; day: number; weekday: number } {
  ...
  return {
    ...
    day: Number(get("day")),
    weekday: DAY_WORDS[get("weekday").toLowerCase()] ?? 0,
  };
}
...
function resolveAttributionReference(token: string, now: Date): { dayKey: string | null; confidence: AttributedBeat["confidence"] } {
  ...
  if (/^(today|this morning|this afternoon|this evening|tonight)$/.test(t)) {
    return { dayKey: attributionDateKey(now), confidence: "relative" };
  }
  if (/^(yesterday|last night)$/.test(t)) {
    return { dayKey: attributionDateKey(attributionShiftDays(now, -1)), confidence: "relative" };
  }
  ...
  if (dayMatch) {
    ...
    const current = attributionSastParts(now).weekday;
