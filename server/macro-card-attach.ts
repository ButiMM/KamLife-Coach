...
async function todayRows(user: any): Promise<Array<{ label: string; current: number; target: number; unit: string }> | null> {
  if (!getGoalProfile(user?.goalType).usesMacros) return null; // wellness → no card
...
export async function macroCardMarker(opts: { user: any; mealName: string; mealKcal: number }): Promise<string> {
  try {
    ...
    if (!base) return "";
    const rows = await todayRows(opts.user);
    if (!rows) return "";
