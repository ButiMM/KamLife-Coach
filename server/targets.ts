...
// ============================================================
// ADAPTIVE CALORIE TARGET — the coach's readjustment brain (2026-07-12, Kam: "the goal
// they set at the beginning — how are we gonna readjust it, safely and sustainably…
// obese clients… clients stuck on belly fat after big loss… surplus clients backing off,
// diet breaks"). The onboarding target (calculateTargets) is clinically sound but STATIC.
// A real coach reviews the trend and adjusts. This does it deterministically and
// CONSERVATIVELY — it only fires on a clear multi-week signal, so an anxious client is
// never whiplashed by weekly target changes. Every recommendation is calm and framed as
// normal, because the market is overwhelmed and scared. Pure — no DB, unit-tested.
//
// Clinical anchors (see docs/clinical-audit.md): sustainable loss is 0.5–1%/week; faster
// than ~1.5%/week risks muscle + isn't sustainable (the obese-client safety case); a
// long-term dieter who stalls needs a diet break (eat at maintenance ~1 week to reverse
// metabolic adaptation), not just a bigger deficit; a muscle-gain surplus that adds
// >0.75%/week is mostly fat and should be trimmed.
// ============================================================
export type CalorieAdjustmentKind =
  | "ease_deficit"      // losing too fast — eat a bit more, protect muscle + sustainability
  | "increase_deficit"  // stalled and not a long-term dieter — nudge the deficit
  | "diet_break"        // long-term dieter stalled — 1 week at maintenance, then resume
  | "trim_surplus"      // gaining too fast — mostly fat, pull the surplus back
  | "add_surplus";      // not gaining — add fuel

export interface CalorieAdjustment {
  kind: CalorieAdjustmentKind;
  newTarget: number;             // the kcal target to move to (for diet_break: the 1-week maintenance intake)
  temporary: boolean;            // diet_break is temporary; the rest are the new steady target
  reason: string;                // calm, plain, anti-anxiety, client-facing
}

export function suggestCalorieAdjustment(opts: {
  goal: string;
  currentTarget: number;
  currentWeightKg: number;
  weeklyRatePct: number;   // signed % of bodyweight per week; negative = losing
  weeksOnProgramme: number;
  calorieFloor?: number;   // safety floor (gender-based); default 1200
}): CalorieAdjustment | null {
  const { goal, currentTarget, currentWeightKg, weeklyRatePct, weeksOnProgramme } = opts;
  const floor = opts.calorieFloor && opts.calorieFloor > 0 ? opts.calorieFloor : 1200;
  if (!Number.isFinite(currentTarget) || currentTarget < 800) return null;
  if (!Number.isFinite(currentWeightKg) || currentWeightKg <= 0) return null;
  if (!Number.isFinite(weeklyRatePct)) return null;
  const g = (goal || "fat_loss").toLowerCase();
  const clamp = (n: number) => Math.max(floor, Math.min(4500, Math.round(n / 50) * 50));

  if (g === "fat_loss" || g === "recomposition") {
    // SAFETY FIRST — losing too fast. Fires from week 2; the obese client who drops weight
    // very quickly at the start needs to be protected, not congratulated into muscle loss.
    if (weeklyRatePct <= -1.5 && weeksOnProgramme >= 2) {
      return {
        kind: "ease_deficit",
        newTarget: clamp(currentTarget + 200),
        temporary: false,
        reason: `You're dropping weight fast — but too fast burns muscle and never lasts. Let's add a little food (to *${clamp(currentTarget + 200)} kcal*) so the weight you lose is FAT and it actually stays off. This is me protecting your results.`,
      };
    }
    // Recomposition holds at maintenance — only the safety case above applies.
    if (g === "recomposition") return null;
    // STALL — barely moving for a few weeks.
    if (weeklyRatePct >= -0.15 && weeksOnProgramme >= 4) {
      if (weeksOnProgramme >= 10) {
        // Long-term dieter (the "lost a lot, now stuck on belly fat" client). A bigger
        // deficit is the WRONG move — the body has adapted. A diet break resets it.
        const maintenance = clamp(currentTarget + 300);
        return {
          kind: "diet_break",
          newTarget: maintenance,
          temporary: true,
          reason: `You've been dieting hard for weeks and the scale's stuck — that's your body adapting, not you failing. For ONE week, eat a bit more (around *${maintenance} kcal/day*). It feels backwards, but it resets your metabolism and hormones, and fat loss restarts after. This is a strategy, not a slip.`,
        };
      }
      return {
        kind: "increase_deficit",
        newTarget: clamp(currentTarget - 150),
        temporary: false,
        reason: `Things have levelled off. Let's trim your target slightly to *${clamp(currentTarget - 150)} kcal* to get the scale moving again — small change, nothing drastic.`,
      };
    }
    return null; // 0.15–1.5%/week loss = sustainable, leave it alone (no friction)
  }

  if (g === "muscle_gain") {
    // Gaining too fast → mostly fat. Pull it back so the gain stays lean.
    if (weeklyRatePct >= 0.75 && weeksOnProgramme >= 3) {
      return {
        kind: "trim_surplus",
        newTarget: clamp(currentTarget - 150),
        temporary: false,
        reason: `You're gaining quickly — faster than muscle can actually build, so some of it is fat. Let's ease your target to *${clamp(currentTarget - 150)} kcal* so the size you add stays lean.`,
      };
    }
    // Not gaining at all → add fuel.
    if (weeklyRatePct <= 0.05 && weeksOnProgramme >= 4) {
      return {
        kind: "add_surplus",
        newTarget: clamp(currentTarget + 150),
        temporary: false,
        reason: `The scale's flat — muscle needs a bit more fuel to grow. Let's bump your target to *${clamp(currentTarget + 150)} kcal* and keep training hard.`,
      };
    }
    return null;
  }
