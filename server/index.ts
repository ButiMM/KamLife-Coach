...
async function runMigrations(): Promise<void> {
  // Schema is managed by drizzle-kit (npm run db:push).
  // This function only checks that the core tables exist and warns if they don't.
  // To create or update tables, run: npm run db:push
  //
  // The old 200+ lines of raw SQL CREATE TABLE statements have been removed.
  // They duplicated schema.ts and conflicted with drizzle-kit migrations.
  const CORE_TABLES = ["users", "chat_history", "weight_logs", "workout_logs", "step_logs"];
  ...
  for (const table of CORE_TABLES) {
    try {
      ...
      logger.warn("migration", `Could not check table ${table}`, e);
    }
  }
  if (missing.length > 0) {
    logger.warn("migration", `Missing tables: ${missing.join(", ")}. Run 'npm run db:push' to create them.`);
  } else {
    logger.info("migration", "All core tables present");
  }

  // ── PHASE 2: Incremental column migrations (additive-only, idempotent) ──
  // These ALTER TABLE statements are safe to run on every startup.
  // They add columns that may have been added after the initial db:push.
  ...
  for (const sql of columnMigrations) {
    try {
      ...
      logger.warn("migration", `Column migration failed: ${sql.slice(0, 60)}...`, e.message);
    }
  }
  logger.info("migration", `Column migrations: ${applied}/${columnMigrations.length} applied`);
}
...
(async () => {
  ...
  app.use((err: any, _req: Request, res: Response, next: NextFunction) => {
    ...
    logger.error("express", "Internal Server Error", err);
  ...
  httpServer.listen(
    ...
    () => {
      ...
      initScheduler();
      initFoodsTable().catch(e => logger.error("startup", "Foods init failed", e));
      initMemoryTable().catch(e => logger.error("startup", "Memory init failed", e));
      initMealLogsTable().catch(e => logger.error("startup", "Meal logs init failed", e));
    },
  );
})();
