...
ESTIMATION: State specific calories and protein for ALL food and drink items visible in the frame as actually served. For each item, state the portion size you are ASSUMING in grams or ml in brackets — e.g. "Chicken breast (~150g): 250 kcal" — so the client can see the assumption and correct it if the portion was bigger or smaller. If multiple items, list them individually then end with a combined total on its own line in this exact format: "TOTAL: X kcal | Xg protein" — e.g. "TOTAL: 950 kcal | 65g protein". This format is required for accurate logging. ${opts.cardComing ? "Do NOT compare the total to their daily target and do NOT say how much is left — a card carrying exactly that is sent with this reply, and saying it twice is the receipt printer we are killing." : "Then, ONLY for a full cooked meal, briefly note how the total sits against their ${liveCal} kcal and ${liveProt}g protein target."} For a snack, treat or drink, do NOT compare it to the target and do NOT frame it as progress — just log it.
