...
- **CI minutes are money.** Fast tests run on GitHub on every push. The ~47-minute database suite does **not** run on GitHub: Codex runs it on the exact head SHA when you add the label `ready` (do that once attacks are answered and fast tests pass), and posts `DBSUITE @ <sha>: PASS|FAIL|UNAVAILABLE`. **Merge only with `DBSUITE @ <head sha>: PASS` from Codex.** That run includes the fast tests, so while GitHub's minutes are exhausted, Codex's PASS stands in for them too. Don't push to a `ready` PR unless the suite failed. Batch fixes into one push. If Codex posts UNAVAILABLE, stop and leave it for the CTO; do not run the suite yourself as a substitute.
